...
export default function CalendarGrid({
  ...
  const trailingDayCount = Math.max(0, 42 - blankDays.length - days.length)

  return (
    <div
      className={cn(
        "grid grid-cols-7 bg-card",
    ...
    >
          ...
          >
            <div className="mb-2 flex h-6 items-start">
              <p
                className={
                  isSameDay(day, today)
                    ? "grid size-6 place-items-center rounded-full bg-primary text-xs leading-none font-medium text-primary-foreground"
                    : "h-6 text-sm leading-6 font-medium"
                }
              >
                {day.getDate()}
              </p>
      ...
      {Array.from({ length: trailingDayCount }, (_, index) => (
        <div key={`trailing-${index}`} className="h-36 border p-2" />
      ))}
    </div>
  )
}
